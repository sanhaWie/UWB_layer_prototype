...
/* 8-1: DMA 정렬 버퍼 (SPI RX → 거리 계산까지 재사용) */
...
static float tag_range_to(uint16_t anchor_addr) {
    ...
    put_u16le(&s_tx_buf[IDX_SRC], ADDR_TAG1);
...
void app_main(void)
{
    ...
    while (1) {
        ...
        /* 2. UWB RX: 연속 수신 대기 (vTaskDelay 없음) */
        ...
        if (!(status & SYS_STATUS_RXFCG_BIT_MASK)) {
            ...
            /* vTaskDelay 없음: 바로 다음 RX → POLL 놓치지 않음 */
            continue;
        }
        dwt_writesysstatuslo(SYS_STATUS_RXFCG_BIT_MASK);
