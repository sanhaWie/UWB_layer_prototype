...
/* ===== 6-1: 삼변측량 (앵커 3개로 XY 계산) ===== */
function trilaterate(anchors, dists) {
    /* anchors: [{x,y}, ...], dists: [mm, mm, mm, mm]
       유효한 거리(>0)가 3개 이상이면 좌표 반환, 아니면 null */
    const valid = [];
    for (let i = 0; i < 4; i++) {
        if (anchors[i] && anchors[i].set && dists[i] > 0) {
            valid.push({ x: anchors[i].x, y: anchors[i].y, r: dists[i] / 1000.0 });
        }
    }
    if (valid.length < 3) return null;

    /* 첫 번째 앵커를 기준으로 연립방정식 풀기 */
    /* (x-x1)²+(y-y1)²=r1², (x-x2)²+(y-y2)²=r2² ...
       두 식의 차이를 취하면 선형 방정식 */
    const A = [];
    const b = [];
    const x1 = valid[0].x, y1 = valid[0].y, r1 = valid[0].r;

    for (let i = 1; i < valid.length; i++) {
        const xi = valid[i].x, yi = valid[i].y, ri = valid[i].r;
        A.push([2 * (xi - x1), 2 * (yi - y1)]);
        b.push(r1*r1 - ri*ri + xi*xi - x1*x1 + yi*yi - y1*y1);
    }
...
/* ===== 6-3: 미디언 필터 (최근 5개 중간값) ===== */
const MEDIAN_SIZE = 5;
let distHistory = [[], [], [], []]; /* 앵커별 최근 거리값 */
...
function filterDistances(rawDist) {
    const filtered = [];
    for (let i = 0; i < 4; i++) {
        if (rawDist[i] > 0) {
            distHistory[i].push(rawDist[i]);
            if (distHistory[i].length > MEDIAN_SIZE) distHistory[i].shift();
        }
        filtered.push(median(distHistory[i]));
    }
    return filtered;
}

/* ===== 태그 데이터 (다중 태그 지원) ===== */
let tags = {};  /* key: device_id, value: tagData */

function getTag(id) {
    if (!tags[id]) {
        tags[id] = {
            id, dist: [-1,-1,-1,-1], distRaw: [-1,-1,-1,-1],
            neo: {r:0,g:0,b:0}, seq: 0, pos: null, lastUpdate: 0,
            distHistory: [[],[],[],[]]
        };
    }
    return tags[id];
...
/* 태그별 미디언 필터 */
function filterDistancesForTag(tag, rawDist) {
    const filtered = [];
    for (let i = 0; i < 4; i++) {
        if (rawDist[i] > 0) {
            tag.distHistory[i].push(rawDist[i]);
            if (tag.distHistory[i].length > MEDIAN_SIZE) tag.distHistory[i].shift();
        }
        filtered.push(median(tag.distHistory[i]));
    }
    return filtered;
}

/* 하위 호환: tagData는 tag1을 가리킴 */
let tagData = null;
...
udpServer.on('message', (msg, rinfo) => {
    ...
    const deviceId = msg.readUInt8(0);
    const tag = getTag(deviceId);
    ...
    tag.distRaw = rawDist;
    tag.dist = filterDistancesForTag(tag, rawDist);
    tag.neo = {
        r: msg.readUInt8(2),
        g: msg.readUInt8(3),
        b: msg.readUInt8(4),
    };
    tag.seq = msg.readUInt16LE(6);
    tag.lastUpdate = Date.now();

    /* 삼변측량 */
    const anchorArr = Object.values(anchors);
    tag.pos = trilaterate(anchorArr, tag.dist);

    /* 하위 호환 */
    tagData = tag;

    broadcast({ type: 'tag', data: tag });
});
...
<div>
    <b>방 크기 (m):</b>
    <input id="roomW" type="number" value="4" min="1" max="20" step="0.1" style="width:50px"> x
    <input id="roomH" type="number" value="4" min="1" max="20" step="0.1" style="width:50px">
    중심: (<input id="centerX" type="number" value="0.6" step="0.1" style="width:50px">,
    <input id="centerY" type="number" value="0.6" step="0.1" style="width:50px">)
</div>
...
<div>
    <table>
        ...
        <tr><td>A1</td>
            ...
            <td id="d1">-</td></tr>
        <tr><td>A2</td>
            <td><input id="a2x" type="number" step="0.01" value="1.2" style="width:60px"></td>
            <td><input id="a2y" type="number" step="0.01" value="0" style="width:60px"></td>
            <td id="d2">-</td></tr>
        <tr><td>A3</td>
            <td><input id="a3x" type="number" step="0.01" value="1.2" style="width:60px"></td>
            <td><input id="a3y" type="number" step="0.01" value="1.2" style="width:60px"></td>
            <td id="d3">-</td></tr>
        <tr><td>A4</td>
            <td><input id="a4x" type="number" step="0.01" value="0" style="width:60px"></td>
            <td><input id="a4y" type="number" step="0.01" value="1.2" style="width:60px"></td>
            <td id="d4">-</td></tr>
    </table>
    ...
    <h4 style="margin:10px 0 5px;">T1 (시안)</h4>
    <table><tr><th>A1</th><th>A2</th><th>A3</th><th>A4</th></tr>
    <tr><td id="t1d1">-</td><td id="t1d2">-</td><td id="t1d3">-</td><td id="t1d4">-</td></tr></table>

    <h4 style="margin:10px 0 5px;">T2 (마젠타)</h4>
    <table><tr><th>A1</th><th>A2</th><th>A3</th><th>A4</th></tr>
    <tr><td id="t2d1">-</td><td id="t2d2">-</td><td id="t2d3">-</td><td id="t2d4">-</td></tr></table>

    <div style="margin-top:8px;">seq: <span id="seq">-</span></div>
...
let anchors = [];
let tagPositions = {}; /* key: id, value: {x,y} */
let tagDist = [-1,-1,-1,-1];
...
function getRoom() {
    return {
        ...
        cx: parseFloat(document.getElementById('centerX').value) || 0.6,
        cy: parseFloat(document.getElementById('centerY').value) || 0.6,
    };
}
...
function m2px(mx, my) {
    ...
    /* 중심점(cx,cy)이 Canvas 중앙에 오도록 오프셋 계산 */
    const canvasCenterX = canvas.width / 2;
    const canvasCenterY = canvas.height / 2;
    const px = canvasCenterX + (mx - room.cx) * sx;
    const py = canvasCenterY - (my - room.cy) * sy; /* Y축 반전 */
    return { x: px, y: py };
}
...
document.querySelectorAll('input[type=number]').forEach(el => {
    el.addEventListener('change', () => { readAnchorsFromInputs(); });
});
...
function draw() {
    ...
    const yMin = room.cy - room.h / 2;
    const yMax = room.cy + room.h / 2;

    ctx.strokeStyle = '#333';
    ctx.lineWidth = 0.5;
    for (let x = Math.ceil(xMin); x <= Math.floor(xMax); x++) {
        const p = m2px(x, yMin);
        const p2 = m2px(x, yMax);
        ctx.beginPath(); ctx.moveTo(p.x, p.y); ctx.lineTo(p2.x, p2.y); ctx.stroke();
    }
    for (let y = Math.ceil(yMin); y <= Math.floor(yMax); y++) {
        const p = m2px(xMin, y);
        const p2 = m2px(xMax, y);
        ctx.beginPath(); ctx.moveTo(p.x, p.y); ctx.lineTo(p2.x, p2.y); ctx.stroke();
    }
    ...
    for (let i = 0; i < anchors.length; i++) {
        ...
        if (tagDist[i] > 0) {
            const rm = getRoom();
            const sx = (canvas.width - 100) / rm.w;
    ...
    /* 다중 태그 위치 표시 */
    const tagColors = ['#0ff', '#f0f', '#ff0', '#0f0'];
    Object.entries(tagPositions).forEach(([id, pos]) => {
        if (!pos) return;
        const p = m2px(pos.x, pos.y);
        const color = tagColors[parseInt(id) % tagColors.length];
        ctx.fillStyle = color;
        ctx.beginPath(); ctx.arc(p.x, p.y, 8, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = '#fff'; ctx.font = 'bold 12px sans-serif';
        ctx.fillText('T' + (parseInt(id)+1), p.x - 8, p.y - 12);
    });
}
...
ws.onmessage = (e) => {
    ...
    if (msg.type === 'tag') {
        ...
        const prefix = 't' + (tid + 1); /* t1 or t2 */
        if (msg.data.pos) {
            tagPositions[tid] = msg.data.pos;
        }
        for (let i = 0; i < 4; i++) {
            const el = document.getElementById(prefix + 'd' + (i+1));
            if (el) {
                el.textContent = msg.data.dist[i] >= 0 ? msg.data.dist[i].toFixed(0) : 'FAIL';
            }
        }
...
/* 7-4: requestAnimationFrame 기반 60fps 렌더링 */
function renderLoop() {
    draw();
...
const server_http = http.createServer((req, res) => {
    if (req.method === 'GET' && req.url === '/') {
        ...
        (async () => {
            /* 1. 모든 태그 STOP */
            const tagIPs = ['192.168.45.22', '192.168.45.242']; /* tag1, tag2 */
            const stopSock = dgram.createSocket('udp4');
            tagIPs.forEach(ip => stopSock.send(Buffer.from([CMD_STOP]), 8082, ip));
            setTimeout(() => stopSock.close(), 100);
            console.log('태그 STOP (' + tagIPs.join(', ') + ')');
            ...
            tagIPs.forEach(ip => resumeSock.send(Buffer.from([CMD_RESUME]), 8082, ip));
            setTimeout(() => resumeSock.close(), 100);
            console.log('태그 RESUME (' + tagIPs.join(', ') + ')');
