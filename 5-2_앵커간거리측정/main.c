...
static float do_range_to(uint16_t target) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        poll[2] = frame_seq_nb++;
        put_u16le(&poll[IDX_PAN], UWB_PAN_ID);
        put_u16le(&poll[IDX_DST], target);
        put_u16le(&poll[IDX_SRC], MY_ADDR);
        poll[IDX_TYPE] = MSG_POLL;
        ...
        if (status & SYS_STATUS_RXFCG_BIT_MASK) {
            ...
            if (resp[IDX_TYPE] == MSG_RESP) {
                ...
                uint32_t poll_rx = get_u32le(&resp[IDX_PAYLOAD + 0]);
                uint32_t resp_tx = get_u32le(&resp[IDX_PAYLOAD + 4]);
...
void app_main(void)
{
    ...
    while (1) {
        ...
        uint8_t cmd_buf[4];
        int n = recv(s_cmd_sock, cmd_buf, sizeof(cmd_buf), 0);
        if (n >= 3 && cmd_buf[0] == CMD_RANGE) {
            /* 레인징 명령: [CMD_RANGE, target_hi, target_lo] */
            uint16_t target = ((uint16_t)cmd_buf[1] << 8) | cmd_buf[2];
            ESP_LOGI(TAG, "레인징 명령 → 0x%04X", target);
            neopixel_set(255, 255, 0); /* YELLOW: 개시자 */
            ...
            continue; /* 명령 처리 후 바로 다음 루프 */
        }

        /* 2. UWB RX: SS-TWR 응답 (태그 + 다른 앵커의 POLL에 응답) */
        ...
        if (!(status & SYS_STATUS_RXFCG_BIT_MASK)) {
            ...
            vTaskDelay(1); /* WiFi/OTA 동작 보장 */
            continue;
        }
        dwt_writesysstatuslo(SYS_STATUS_RXFCG_BIT_MASK);

        uint8_t rx_buf[RESP_LEN];
        dwt_readrxdata(rx_buf, POLL_LEN, 0);
        uint16_t dst = (uint16_t)rx_buf[IDX_DST] | ((uint16_t)rx_buf[IDX_DST + 1] << 8);
        if (dst != MY_ADDR) continue;

        if (rx_buf[IDX_TYPE] == MSG_POLL) {
            handle_ss_twr_poll(rx_buf);
        }
    }
}
